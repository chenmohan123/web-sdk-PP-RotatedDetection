...
import { drawPolygons, polygonAtPoint } from "./draw-polygon";
...
const labels = {
  zh: {
    ...
    downloading: "正在下载模型",
    cache_reading: "正在读取缓存",
    cache_hit: "缓存已命中",
    cache_miss: "无缓存，准备下载",
    cache_invalid: "缓存损坏，重新下载",
    score: "置信度阈值",
    nms: "旋转 NMS 阈值",
    exportJson: "导出 JSON",
    exportPng: "导出叠加图",
  ...
  en: {
    ...
    empty: "Upload or drop an image to reveal rotated boxes",
    ...
    cache_reading: "Reading cache",
    cache_hit: "Cache hit",
    cache_miss: "Cache miss, downloading",
    cache_invalid: "Invalid cache, downloading again",
    score: "Score threshold",
    nms: "Rotated NMS threshold",
    exportJson: "Export JSON",
    exportPng: "Export overlay",
    ...
    resultEmpty: "Objects appear here after detection",
...
export default function App() {
  ...
  const sdk = useRef<RotatedDetection | null>(null),
  ...
  useEffect(() => {
    if (preview && canvas.current)
      drawPolygons(canvas.current, preview, result, selected, overlay);
  ...
  async function run() {
    ...
    setResult(undefined);
    setSelected(-1);
    let owned: RotatedDetection | undefined;
    try {
      await release.current;
      const module = await getApi();
      if (id !== generation.current) return;
      owned = module.createRotatedDetection({
        model: model(),
        backend,
        executionMode: mode,
      ...
      await owned.load({
        ...
        onProgress: (event) => {
          if (id === generation.current) {
            setPhase(
              event.phase === "cache"
                ? `cache_${event.cacheStatus ?? "reading"}`
      ...
      void updateCache(id);
      setPhase("running");
      const output = await owned.run(
        { image: file },
        { signal: c.signal, scoreThreshold, nmsThreshold },
      );
      if (id === generation.current) {
        setResult(output);
        setPhase("success");
  ...
  function exportPng() {
    ...
    drawPolygons(target, preview, result, -1, true);
    target.toBlob((blob) => {
      if (blob) download(blob, "rotated-detection.png");
    });
  }
  const displayPhase = previewError
    ? "error"
    : phase.startsWith("cache_")
      ? "loading"
      : phase;
  ...
  const stateText =
    file && !preview && !previewError
      ? t.previewLoading
      : ((t as Record<string, string>)[notice || phase] ?? displayPhase);
  return (
    <div className="shell">
      <header className="topbar">
        <div className="brand-block">
          <span className="eyebrow">ONNX RUNTIME WEB</span>
          <h1>PP-RotatedDetection</h1>
          <span className="version">SDK 0.1.0</span>
        </div>
        <nav className="top-actions">
          <a
            className="repository-link"
            href="https://github.com/chenmohan123/web-sdk-PP-RotatedDetection"
            target="_blank"
            rel="noreferrer"
          >
            <Github size={16} />
          ...
          <section className="control-band">
            ...
            <label className="control-group">
              <span className="control-label">
                {t.score} · {scoreThreshold.toFixed(2)}
              </span>
              <input
                type="range"
                aria-label={t.score}
            ...
            <label className="control-group">
              ...
              <input
                ...
                disabled={busy || clearing}
                onChange={(e) => {
                  setNmsThreshold(Number(e.target.value));
                  reset();
                }}
              />
            </label>
        ...
        <section className="viewer">
          <div className="viewer-toolbar">
            ...
            <span className="selection" title={file?.name}>
              {selected >= 0
                ? `#${selected + 1} · ${result?.detections[selected]?.label ?? ""}`
                : (file?.name ?? "")}
            </span>
            <button
          ...
          >
            {url ? (
              <canvas
                ...
                aria-label={t.title}
                onClick={(e) => {
                  if (!result || !canvas.current || !overlay) return;
                  const rect = canvas.current.getBoundingClientRect();
                  const scale = Math.min(
                    rect.width / result.image.width,
                    rect.height / result.image.height,
                  );
                  const x =
                    (e.clientX -
                      rect.left -
                      (rect.width - result.image.width * scale) / 2) /
                    scale;
                  const y =
                    (e.clientY -
                      rect.top -
                      (rect.height - result.image.height * scale) / 2) /
                    scale;
                  setSelected(polygonAtPoint(result.detections, x, y));
                }}
              />
            ) : (
              <button
          ...
          <section className="results detail-section">
            <div className="panel-title">
              ...
              <span className="count-badge">
                {result?.detections.length ?? 0} {t.count}
              </span>
            </div>
              ...
              <button
                ...
                onClick={() =>
                  ...
                  download(
                    ...
                    "rotated-detection.json",
                  )
                }
              >
                {t.exportJson}
              </button>
              <button disabled={!result} onClick={exportPng}>
                {t.exportPng}
              </button>
            </div>
            <div className="result-list">
              ...
              {!result?.detections.length && (
                <p className="result-empty">
                  {result ? t.noObjects : t.resultEmpty}
                </p>
              )}
            </div>
          </section>
          ...
          <details className="detail-section">
            ...
            <div className="info-grid">
              <div data-sdk-model-info>
                <b>PP-YOLOE-R-s · FP32</b>
                <p>33.16 MB · 8.244M · ONNX opset 17</p>
                <p>1024 × 1024 · DOTA 15</p>
                <p>
              ...
              <div data-sdk-runtime-info>
                ...
                <p>
                  {lang === "zh"
                    ? "40 项桌面公共 API 数值对照通过；非全量 DOTA mAP。"
                    : "40 desktop public API numerical checks passed; not full DOTA mAP."}
                </p>
                {result && (
                  <p>
