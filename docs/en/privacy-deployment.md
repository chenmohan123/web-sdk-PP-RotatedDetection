...
Production contains no weights, evaluation images or local model URL. Models are downloaded from the explicitly selected manifest Hub source. Only explicit dev-local/vanilla-local development servers read `.tmp/model.onnx`. Pages builds protected main, checks both sources and production receipts with `node scripts/check-release-ready.mjs`, then uploads the artifact. Deployment uses the github-pages environment, minimal permissions and serialized concurrency. Remote protection, Pages Source, HTTPS and successful deployment require API receipts; see [Release](release.md) for current status.
