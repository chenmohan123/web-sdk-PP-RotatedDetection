import type {
  RotatedDetectionOptions,
  RotatedDetection,
  LoadOptions,
  RunOptions,
  RotatedDetectionInput,
...
function validateOptions(options: RotatedDetectionOptions): void {
  ...
  if (
    ...
    throw new RotatedDetectionError("INVALID_INPUT", "未知执行后端");
  if (
    options.executionMode !== undefined &&
    !["main", "worker"].includes(options.executionMode)
  )
    throw new RotatedDetectionError("INVALID_INPUT", "未知执行模式");
}
export function createRotatedDetection(
  options: RotatedDetectionOptions,
): RotatedDetection {
  ...
  function guard(signal?: AbortSignal) {
    if (disposed) throw new RotatedDetectionError("DISPOSED", "实例已释放");
    checkAbort(signal);
    if (active)
      throw new RotatedDetectionError("BUSY", "实例正忙，请等待当前操作完成");
  }
  function operate<T>(
  ...
  async function download(
      ...
      try {
        while (true) {
          ...
          if (offset + value.length > data.length)
            throw new RotatedDetectionError("INTEGRITY", "模型字节数超过清单");
    ...
    if (offset !== model.bytes)
      throw new RotatedDetectionError("INTEGRITY", "模型字节数与清单不符");
    return data;
  }
  return {
    manifest: model,
    ...
    async run(
      ...
      if (!ready || !runner)
        throw new RotatedDetectionError("NOT_LOADED", "请先加载模型");
      validateRunOptions(runOptions);
      return operate(runOptions.signal, async (signal) => {
        const start = performance.now();
        try {
          if (!input?.image)
            throw new RotatedDetectionError("INVALID_INPUT", "缺少输入图片");
          ...
          throw wrapError(error, "INFERENCE", "旋转框检测推理失败");
        }
      });
    },
