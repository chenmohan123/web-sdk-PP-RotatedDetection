...
import type {
  ...
  RotatedDetectionErrorCode,
} from "./types";
import { RotatedDetectionError, checkAbort, wrapError } from "./errors";
import { preprocess } from "./preprocess";
import { postprocess } from "./postprocess";
import { loadOrt } from "./ort";
export type CoreResult = Pick<
  RotatedDetectionResult,
  "image" | "detections" | "timings"
>;
...
class MainRunner implements Runner {
  ...
  async load(data: Uint8Array, signal?: AbortSignal) {
    ...
    if (
      ...
      this.session.outputNames.length !== 2
    )
      throw new RotatedDetectionError(
        "INVALID_MANIFEST",
        "模型输入输出不符合PP-YOLOE-R 双输出契约",
      );
  }
  async run(
    image: PixelImage,
    ...
    if (!this.ort || !this.session)
      throw new RotatedDetectionError("NOT_LOADED", "模型尚未加载");
    ...
    try {
      ...
      // 导出名称由转换器生成；固定形状定义 scores/rboxes 语义。
      const values = shapes.map((shape) => {
        const matches = this.session!.outputNames.map(
          (name) => outputs![name],
        ).filter(
          (t) =>
            t &&
            t.dims.length === shape.length &&
            t.dims.every((v, j) => v === shape[j]),
        );
        if (matches.length !== 1 || !(matches[0].data instanceof Float32Array))
          throw new RotatedDetectionError(
            "INFERENCE",
            "模型输出形状或类型错误",
          );
...
interface Reply {
  id: number;
  result?: CoreResult;
  error?: { code?: RotatedDetectionErrorCode; message: string };
}
class WorkerRunner implements Runner {
  ...
  constructor(private options: RunnerOptions) {
    ...
    this.worker.onerror = () =>
      this.fail(
        new RotatedDetectionError("SESSION", "推理Worker加载或执行失败"),
      );
    this.worker.onmessageerror = () =>
      this.fail(
        new RotatedDetectionError("INFERENCE", "推理Worker消息解析失败"),
      );
  }
  ...
  private call(
    ...
    if (this.dead)
      return Promise.reject(
        new RotatedDetectionError("DISPOSED", "Worker已释放"),
      );
    const id = ++this.next;
  ...
  async dispose() {
    if (!this.dead)
      this.fail(new RotatedDetectionError("ABORTED", "推理Worker已终止"));
  }
}
