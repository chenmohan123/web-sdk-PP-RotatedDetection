...
import { createRotatedDetection } from "../src/runtime";
...
const make = () => createRotatedDetection({ model, executionMode: "main" });
function gate() {
  let resolve!: () => void;
...
it("拒绝非法清单、未知后端和执行模式", () => {
  ...
  expect(() =>
    createRotatedDetection({ model, backend: "npu" as never }),
  ).toThrow();
  expect(() =>
    createRotatedDetection({ model: { ...model, url: "file:///model" } }),
  ).toThrow();
});
...
it("在途运行取消后丢弃结果并可恢复", async () => {
  ...
  expect((await sdk.run({ image })).detections).toEqual([]);
  await sdk.dispose();
});
